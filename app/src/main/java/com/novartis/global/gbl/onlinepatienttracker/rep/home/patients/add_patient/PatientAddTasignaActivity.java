...
public class PatientAddTasignaActivity extends AppCompatActivity {
    ...
    Spinner citySpinner, hospitalSpinner, doctorSpinner, lineSpinner, managementSpinner, doseSpinner;
    ImageView back;
    TextView addBtn;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_patient_add_tasigna);
    ...
    private void addPatient() throws JSONException {
        ...
        map.put("cml_type", String.valueOf(lineSpinner.getSelectedItemPosition()));
        map.put("current_management", managementSpinner.getSelectedItem().toString().toLowerCase());
        if (selectedManagement == 5) {
            map.put("dose", doseSpinner.getSelectedItem().toString().toLowerCase());
        }
...
//        map.put("brand_id", String.valueOf(brandId));
        ...
        dialog.show();
        Webservice.getInstance().getApi().addTasignaPatient(accessToken, map).enqueue(new Callback<ResponseBody>() {
            @Override
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                ...
                if (response.code() == 200) {
                    JSONObject responseObject = null;
                    try {
                        Toast.makeText(PatientAddTasignaActivity.this, "data added successfully", Toast.LENGTH_SHORT).show();
                        onBackPressed();
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
    ...
    private boolean validateFields() {
        if (citySpinner.getSelectedItemPosition() == 0 || hospitalSpinner.getSelectedItemPosition() == 0
                ...
                || managementSpinner.getSelectedItemPosition() == 0) {
            Toast.makeText(this, "Please Fill all fields", Toast.LENGTH_SHORT).show();
            return false;
        } else if (managementSpinner.getSelectedItemPosition() == 5 && doseSpinner.getSelectedItemPosition() == 0) {
            Toast.makeText(this, "Please Fill all fields", Toast.LENGTH_SHORT).show();
            return false;
        } else {
            return true;
